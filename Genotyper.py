def genotype(sex, albino, color, spotting, tremor):
    ...
    if albino == True:
        ...
        color = ["Black", "Chcolate", "Gray", "Lilac"]
    ...
    if sex == "Female":
        ...
        rand = [random.randint(0,1), random.randint(0,1), random.randint(0,2)]
        if tremor == True: bun_trem = trem_gen[3]
        if tremor == False: bun_trem = trem_gen[rand[2]]
        print("Sex = ", bun_sex, "\nTremor = ",  bun_trem) #TS
    if sex == "Male":
        bun_sex = [sex_gen[0], sex_gen[1]]
        rand = [random.randint(0,1), random.randint(1,2), random.randint(0,2)]
        if tremor == True: bun_trem[0] = "Xpt"
        if tremor == False: bun_trem[0] = "X+"
        bun_trem[1] = "N/A"
        print("Sex = ", bun_sex, "\nTremor = ",  bun_trem) #TS
...
mom_p = ["Female", False, "Gray", "Charlie", False]
mom = genotype(mom_p[0], mom_p[1], mom_p[2], mom_p[3], mom_p[4])
print("Mom is \n", mom)

dad_p = ["Male", False, "Chocolate", "Solid", True] 
dad = genotype(dad_p[0], dad_p[1], dad_p[2], dad_p[3], dad_p[4])
